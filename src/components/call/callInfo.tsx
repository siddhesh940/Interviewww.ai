...
function CallInfo({
  ...
  useEffect(() => {
    ...
    if (call && name) {
      const formattedTranscript = replaceAgentAndUser(call?.transcript as string, name);
      setTranscript(formattedTranscript);
      
      // Perform sentiment analysis on the user's responses
      const userResponses = call.transcript_object
    ...
    <div className="h-screen z-[10] mx-2 mb-[100px] overflow-y-scroll">
      {isLoading ? (
        ...
        <>
          ...
          {sentimentAnalysis && (
            <SentimentAnalysisSection analysisResult={sentimentAnalysis} />
          )}
